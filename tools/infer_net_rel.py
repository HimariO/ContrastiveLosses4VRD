...
from core.test_engine_rel import run_inference, initialize_model_from_cfg
...
def main():
    ...
    img = cv2.imread('data/test_img.jpeg')
    model = initialize_model_from_cfg(args, gpu_id=0)
    detect = im_detect_rels(model, img, None, None)
    detect['image'] = 'data/test_img.jpeg'
    print(detect)
    task_evaluation_vg_and_vrd.eval_rel_results([detect], args.output_dir, topk=100, do_val=False)
